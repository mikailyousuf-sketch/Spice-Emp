...
export async function GET(request: Request) {
  ...
  const { data: attempt } = await admin
    ...
    .select("id,provider_checkout_id,provider_reference")
  ...
  const reference = attempt?.provider_checkout_id || attempt?.provider_reference;
  ...
  try {
    const verification = await new YocoProvider().verifyPayment(reference);
