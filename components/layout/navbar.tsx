...
function IconCart() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="size-5" fill="none" stroke="currentColor" strokeWidth="1.7">
      <path d="M3.5 5h2l1.6 9.2a2 2 0 0 0 2 1.7h7.8a2 2 0 0 0 1.9-1.4L21 8H6.2" />
    ...
    <>
      <header className="pointer-events-none fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6 sm:pt-5">
        <div className="mx-auto flex max-w-[1400px] items-start justify-between">
          <Link
            href="/"
            className="pointer-events-auto brand-float brand-float-logo"
            aria-label="The Glided Pantry home"
          >
            <img
          ...
          <div className="pointer-events-auto flex items-center gap-2">
            <Link href="/shop#pantry-search" className="float-control" aria-label="Search pantry">
              <IconSearch />
            </Link>
            <Link href="/cart" className="float-control" aria-label="Cart">
              <IconCart />
            </Link>
            <button
              type="button"
      ...
      <aside className={`menu-panel ${menuOpen ? "is-open" : ""}`} aria-hidden={!menuOpen}>
        <div className="flex items-center justify-between">
          <img src="/branding/glided-monogram.webp" alt="" className="h-16 w-auto opacity-95" />
          <button type="button" className="float-control" onClick={() => setMenuOpen(false)} aria-label="Close menu">
            <IconMenu open />
          </button>
        </div>
        ...
        <nav className="mt-16 grid gap-3">
          {navLinks.map((link, index) => (
            <Link
              ...
              className="menu-link"
            >
              <span className="text-xs tabular-nums text-white/35">0{index + 1}</span>
        ...
        <p className="mt-auto pt-12 text-xs uppercase tracking-[.2em] text-white/35">
          South Africa · Retail + Wholesale
        </p>
      </aside>
    </>
  );
